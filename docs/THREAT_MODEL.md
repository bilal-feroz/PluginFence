...
| **Where is the key?** | In the IDE credential store (`PasswordSafe`), never in a settings file. `OPENAI_API_KEY` from the environment is accepted as a fallback. It is sent only as a bearer token to the endpoint the user configured. |
