...
| `OpenAiCompatibleClient` | JDK `HttpClient` + the platform's bundled Gson. Chat Completions with `tools`; tolerant of servers that return tool arguments as objects. Retries 429/5xx, honouring `Retry-After` or the provider's "try again in Xs" (free tiers are tight - Groq allows 8k tokens/minute - and a half-finished investigation is worse than a short pause). Can pin `tool_choice` to one function, which is how a model that drifted into prose is made to deliver a structured answer. No streaming: the tool calls are the progress indicator. |
| `AiProvider` | Presets for OpenAI, Groq, Ollama and custom endpoints: base URL, a default model known to handle the tool protocol, and which environment variable holds the key. Switching provider changes no code - the point of speaking one wire format. |
| `AnalysisService` | Application service. Runs analyses **one at a time** (parallel investigations are the fastest way to exhaust a provider's per-minute budget), caches results per task key, publishes progress on the message bus (`AnalysisListener`), applies recommendations through `FenceEngine.setPolicy` / `PolicyStore.approve\|revoke`, and - when enabled - analyses the first CRITICAL incident per plugin. It owns the failover chain: configured model → backup model on the same endpoint → `DeterministicAnalyst`. |
| `DeterministicAnalyst` | The analysis PluginFence can always produce: verdict, narrative, evidence and recommendations derived from the correlated chain, the risk factors, the baseline and the drift report - no model, no key, no network. Used when nothing is configured, when every model fails, and as the honest answer on an exhausted free tier. It is why the analyst has no error state. |
